# Unbound DNS Server Minimal Docker Image
A minimalistic Alpine-based Docker image for running Unbound. Instead of compiling Unbound, this image uses the Unbound package from the official Alpine repository.
...
ghcr.io/majorp93/unbound-minimal:latest
```

By default, this image forwards queries Cloudflare DNS server over TLS. In other words, it does not act as a recursive server. The unbound.sh provides the configuration unless it is overriden as described below.

*Note: The example unbound.conf file is different from the one set by unbound.sh. The example is provided to help you re-configure this as a [recursive server](https://github.com/majorp93/unbound-minimal-docker-minimal#recursive-config).*
...
You can create your own configuration file and override the one placed in `/etc/unbound/forward-records.conf` in the container. This is useful if you prefer to use something other than Cloudflare DNS but do not want to provide a custom unbound.conf file.
...
--publish=53:53/udp \
--restart=unless-stopped \
--volume $(pwd)/forward-records.conf:/etc/unbound/forward-records.conf:ro \
ghcr.io/majorp93/unbound-minimal:latest
```
...
--publish=53:53/udp \
--restart=unless-stopped \
--volume=/my-directory/unbound:/etc/unbound/ \
ghcr.io/majorp93/unbound-minimal:latest
```
...
```
include: /etc/unbound/local-zone-unbound.conf
```
...
--detach=true \
--restart=unless-stopped \
--volume=$(pwd)/my-directory/forward-records.conf:/etc/unbound/forward-records.conf:ro \
--volume=$(pwd)/my-directory/a-records.conf:/etc/unbound/a-records.conf:ro \
ghcr.io/majorp93/unbound-minimal:latest
```
...
--publish=53:53/udp \
--restart=unless-stopped \
--volume $(pwd)/a-records.conf:/etc/unbound/a-records.conf:ro \
ghcr.io/majorp93/unbound-minimal:latest
```
...
--publish=53:53/udp \
--restart=unless-stopped \
--volume $(pwd)/srv-records.conf:/etc/unbound/srv-records.conf:ro \
ghcr.io/majorp93/unbound-minimal:latest
```
...
services:
  unbound:
    ...
    image: "ghcr.io/majorp93/unbound-minimal:latest"
    expose:
      - "53"
    networks:
      - dns
    ports:
      ...
      - "53:53/udp"
    volumes:
      - "/data/unbound/my_conf/forward-records.conf:/etc/unbound/forward-records.conf"
      - "/data/unbound/my_conf/a-records.conf:/etc/unbound/a-records.conf"
    restart: unless-stopped
networks:
  dns:
```
...
The default config forwards forwards DNS queries to another DNS server over TLS. If you would rather this work as a recursive DNS server, you must [use a customized Unbound configuration](https://github.com/majorp93/unbound-minimal-docker#use-a-customized-unbound-configuration). An [example unbound.conf](https://github.com/majorp93/unbound-minimal-docker/blob/master/unbound.conf) file to configure unbound as a recursive server is available as a guide.
...
Documentation for this image is stored right here in the [`README.md`](https://github.com/majorp93/unbound-minimal-docker/blob/master/README.md).
...
through a [GitHub issue](https://github.com/majorp93/unbound-minimal-docker/issues).
...
file](https://github.com/majorp93/unbound-minimal-docker/blob/master/README.md)
before attempting a pull request.

Before you start to code, I recommend discussing your plans through a [GitHub
issue](https://github.com/majorp93/unbound-minimal-docker/issues), especially for
...
file](https://github.com/majorp93/unbound-minimal-docker/blob/master/LICENSE) for
details.
